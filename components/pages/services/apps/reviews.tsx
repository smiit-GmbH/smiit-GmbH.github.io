...
function ReviewSlide({
  ...
  return (
    <article className="grid grid-cols-1 items-start gap-8 md:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)] md:items-center md:gap-12">
      <div className="flex flex-col">
        ...
        {review.metric && (
          <CountUpMetric
            ...
            className="mt-3 font-serif font-semibold leading-[0.95] tracking-tight text-[#F703EB] text-[clamp(2.8rem,13vw,5rem)]"
          />
        )}
        {review.metricSub && (
      ...
      <div className="flex flex-col">
        <blockquote className="border-l-2 border-[#F703EB]/40 pl-5 font-serif italic text-[1.02rem] leading-[1.55] tracking-tight text-[#0B162D] sm:text-[1.12rem]">
...
export default function AppsReviews({ dict, lang }: { dict: any; lang: Locale }) {
  const heading = useRevealOnScroll()
  const stage = useRevealOnScroll({ margin: "-80px" })
  const reduceMotion = useReducedMotion() ?? false

  const reviews: Review[] = (dict.servicesApps.reviews ?? []).map((r: any) => ({
    ...r,
  ...
  const ariaLabel = dict.servicesApps.eyebrows?.reviews ?? "Reviews"
        ...
        <div ref={stage.ref} className={`mt-12 reveal-fade-up ${stage.isRevealed ? "revealed" : ""}`}>
          ...
          <div className="lg:hidden">
            <MobileReviewsCarousel
              ...
              reduceMotion={reduceMotion}
              ariaLabel={ariaLabel}
              swipeHint={dict.servicesApps.reviewsHeading?.swipeHint ?? "wischen →"}
              lang={lang}
            />
          </div>
              ...
              return (
                ...
                >
                  <div className="flex flex-col">
                    ...
                    {r.metric && (
                      <span className="mt-3 font-serif text-[4.2rem] font-semibold leading-[0.95] tracking-tight text-[#F703EB] sm:text-[5.5rem] lg:text-[6rem]">
                        {r.metric}
                      </span>
                    )}
                    {r.metricSub && (
                  ...
                  <div className="flex flex-col">
                    <blockquote className="border-l-2 border-[#F703EB]/40 pl-5 font-serif italic text-[1.05rem] leading-[1.55] tracking-tight text-[#0B162D] sm:text-[1.15rem] sm:leading-[1.55] lg:text-[1.2rem]">
                      &ldquo;{r.quote}&rdquo;
                    </blockquote>
